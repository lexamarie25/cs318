...
function Temple () {
	...
	this.isAutoplayAvailable = function() {
		...
		if (!this.isiOS) {
			...
			if(this.isiPad)
				var os = Number(navigator.userAgent.split('iPad')[1].split(' ')[3].replace('_','.')); // check if running on safari iOS 10+
			else
				var os = navigator.userAgent.split('iPhone OS ')[1].split(' ')[0].split('_')[0]; // check if running on safari iOS 10+
			if(os >= 10)
				return true
			else
				return false;
	...
	this.utils = (function() {
		...
		var fitText = function(t) {
			...
			if (t.offsetHeight > p.offsetHeight) {
				while (t.offsetHeight > p.offsetHeight) {			
					s -= .2;
		...
		return {
			...
			findElements: 	function(e){ return findElements(e); }
		}
	})();
...
temple.core.EventDispatcher = ( function() {	
	...
	EventDispatcher.prototype.moduleReady = function() {
		temple.utils.debug('Module <' + this.constructor.name+ ' v' + (this.version||1) + '>', this.color || 'Tomato');
...
temple.core.Core = ( function(_super) {
	...
	Core.prototype.pageReady = function() {
		this.loadConfig();
		this.addEventListener(temple.events.EXIT, this.onExit.bind(this));
	}
	...
	Core.prototype.loadConfig = function() {
	    ...
	    temple.utils.loadJSON(config, function(json) {
			...
			this.config.scriptPath = this.config.scriptPath || '../scripts/temple/';
	...
	Core.prototype.runChain = function(e) {
		...
		if (this.chained[0].m.prototype.__proto__.constructor != temple.core.EventDispatcher) {
			this.chained[0].m.call(this,this.chained[0].a);
			this.chainLoaded();			
			return;
		}
		...
		this[moduleName] = new this.chained[0].m(this.chained[0].a||this,this,t||0);
		this[moduleName].addEventListener(temple.events.READY, this.chainLoaded.bind(this));
		this[moduleName+'List'] = this[moduleName+'List'] || [this[moduleName]];
		this[moduleName+'List'].push(this[moduleName]);
	}
	...
	Core.prototype.politeLoads = function(c) {
	    ...
	    var combined = new XMLHttpRequest();
        
		combined.onload = function(e) {
			if(e.target.status == 200) {
				var parser = new DOMParser();
				var svgObj = {};
				var svgString = e.currentTarget.response.split('<svg');
				svgString.shift();
				for(var i = 0; i < svgString.length; i++) {
					var svg = parser.parseFromString('<svg'+svgString[i], 'text/xml').getElementsByTagName('svg')[0];
					svgObj[svg.getAttribute('name')] = svg;
				}

				for (i = 0; i < s.length; i++) {
					var nm = s[i].getAttribute('data-src').split('/');
					nm = nm[nm.length - 1];
					var r = svgObj[nm];
					var cl = s[i].getAttribute('class');
					var id = s[i].getAttribute('id');
					var node = r.cloneNode(true);
					if(cl) {
						r.setAttribute('class', cl);
					}
					if(id) {
						r.setAttribute('id', id);
					}
					s[i].parentNode.replaceChild(r.cloneNode(true), s[i]);
					if(id) {
						window[id] = node;
					}
					t2++;
        ...
        if(s.length > 0 && this.config.combine) {
		    combined.open('GET', 'img/combined.svg', false);
		    combined.overrideMimeType("image/svg+xml");
		    combined.send("");
        } else {
        	loadDefaultSvg();
        }
...
temple.platforms.dc.Platform = ( function(_super){ 
	...
	Platform.prototype.exit = function(type, url) {
		this.dispatchEvent(temple.events.EXIT,url||this.defaultExitURL||this.exitURLs[0]);
		Enabler.exit('Default Exit', url||this.defaultExitURL||this.exitURLs[0]);
	}
...
temple.templates.StandardBanner = ( function( _super ) {
    ...
    StandardBanner.prototype.ready = function ready() {
        ...
        if ( document.querySelector('body').classList.contains('snapshot')) {
            this.config.language = window.snapshot_language;
        }

        this.politeLoads(this.init.bind(this));
            this.setLanguage(function() {
        }.bind(this));
    }    

    StandardBanner.prototype.show = function() {
    ...
    StandardBanner.prototype.runStartAnimation = function() {
...
temple.Banner = function(_super) {
    ...
    banner.prototype.init = function() {
        this.chain(temple.modules.MonetModule, {
            backup: this.config.monet.json
        });
        this.chain(temple.modules.NetflixCtaModule, {
            element: ctaButtonModule,
            size: 12
        });
        this.chain(this.setDynamicVarsCustom);
        this.chain(this.setInitialValues);
        this.chain(this.addListeners);
        this.chain(this.show);
    };
    banner.prototype.setDynamicVarsCustom = function() {
        this.dimensions = String(this.config.size.width + "x" + this.config.size.height);
        this.retinaDimensions = this.config.retina ? String(this.config.size.width * 2 + "x" + this.config.size.height * 2) : String(this.config.size.width + "x" + this.config.size.height);
        if (this.config.retina) {
            this.config.size.width = this.config.size.width * 2;
            this.config.size.height = this.config.size.height * 2;
        }
        this.showReplacementImage = this.config.replacement_image;
        this.replacementImage = this.config.replacement_image_url;
        if (this.monetModule.backupDataLoaded && this.showReplacementImage) {
            $("#replacement_Image").src = this.replacementImage;
        } else {
            this.templateBlack = this.monetModule.data["rootAssets"]["bool.Template_Black"].value;
            this.geoHidden = this.monetModule.data["rootAssets"]["text.Country"].text.toLowerCase();
            this.Text_RightToLeft = this.monetModule.data["rootAssets"]["bool.Text_RightToLeft"].value;
            this.ctaText = this.monetModule.data["rootAssets"]["text.CTA_Text"].text;
            $(".disclaimer").innerHTML = this.monetModule.data["rootAssets"]["text.Disclaimer"].text;
            $(".ratingBug").src = this.monetModule.data.assetGroups[0]["image.Ratings_Bug_14x14"].url;
            $(".titleImage").src = this.monetModule.data.assetGroups[0]["image.Single_Image_" + this.dimensions].url;
        }
        this.Exit_URL_Desktop = this.monetModule.data["rootAssets"]["url.Exit_URL_Desktop"].url;
        this.Exit_URL_iOS = this.monetModule.data["rootAssets"]["url.Exit_URL_iOS"].url;
        this.Exit_URL_Android = this.monetModule.data["rootAssets"]["url.Exit_URL_Android"].url;
    ...
    banner.prototype.addListeners = function() {
        if (this.monetModule && this.config.monet) {
            this.monetModule.addTrackingListeners();
        }
        this.hidden = false;
    };
    banner.prototype.setInitialValues = function() {
        this.bannerElements = this.findElements($("#banner"));
        if (temple.isSafari) {
            this.setClass("isSafari", true);
        }
        this.setClass(this.geoHidden, true);
        this.setAllCopy();
        if (this.Persistent_Ratingbug) {
            TweenLite.set(".ratingBug", {
                opacity: 1,
                "z-index": 1e3
            });
        }
        if (this.Text_RightToLeft) {
            this.setClass("rtl", true);
        }
        if (!this.templateBlack) {
            this.setClass("templateWhite", true);
        }
        if (this.monetModule.backupDataLoaded && this.showReplacementImage) {} else {
            this.netflixctaModule.setText(this.ctaText.toUpperCase());
        }
        this.masterTimeline = new TimelineMax({
            paused: true,
            delay: .2
        });
        this.masterTimeline.add("start").to(".leftPillar", .2, {
            height: 0,
            ease: Power2.easeIn
        }, "start").to(".middlePillarFill", .2, {
            y: 430,
            ease: Power2.easeIn
        }).to(".rightPillar", .2, {
            height: 0,
            ease: Power2.easeIn
        }).add("panelSlide", "-=0.30").from(".redpanel", 1.1, {
            y: 30,
            ease: Power2.easeOut
        }, "panelSlide").from(".panel", 1.1, {
            y: 97,
            ease: Power2.easeOut
        }, "panelSlide");
        this.copyContainerHeight = parseInt($("#mainCopyContainer").clientHeight);
        this.copyHeight = parseInt($("#mainCopy").clientHeight);
        this.blankSpace = Math.round((this.copyContainerHeight - this.copyHeight) / 2);
        TweenMax.set(".disclaimer", {
            y: -this.blankSpace
        });
        if (temple.isMobile) {
            if (temple.isiOS) {
                this.defaultExitURL = this.Exit_URL_iOS;
            } else {
                this.defaultExitURL = this.Exit_URL_Android;
            }
        } else {
            this.defaultExitURL = this.Exit_URL_Desktop;
        }
        this.show();
    };
    ...
    banner.prototype.onOver = function() {
        this.netflixctaModule.onOver();
    };
    banner.prototype.onOut = function() {
        this.netflixctaModule.onOut();
    };
    banner.prototype.setAllCopy = function() {
        this.setCopy(mainCopy, mainCopyContainer);
    };
    banner.prototype.setCopy = function(copy, container) {
        ...
        copy = !copy.length ? [ copy ] : copy;
        container = !container.length ? [ container ] : container;
        for (i = 0; i < copy.length; i++) {
            var fontSize = parseInt(window.getComputedStyle(copy[i], null).getPropertyValue("font-size"));
            var lineHeight = Math.max(0, parseInt(window.getComputedStyle(copy[i], null).getPropertyValue("line-height")));
            while (copy[i].offsetWidth > container[i].offsetWidth && fontSize > 1) {
                fontSize -= 1;
                lineHeight -= 1;
                copy[i].style.fontSize = fontSize + "px";
                copy[i].style.lineHeight = lineHeight + "px";
            }
            while (copy[i].offsetHeight > container[i].offsetHeight && fontSize > 1) {
                fontSize -= 1;
                lineHeight -= 1;
                copy[i].style.fontSize = fontSize + "px";
                copy[i].style.lineHeight = lineHeight + "px";
    ...
    banner.prototype.setClass = function(className, add) {
        if (add) {
            TweenLite.set(this.bannerElements, {
                className: "+=" + className
            });
        } else {
            TweenLite.set(this.bannerElements, {
    ...
    banner.prototype.onExit = function(type) {
        this.masterTimeline.progress(1);
    };
    banner.prototype.findElements = function(element) {
        var ar = [];
        findElement(element);
        function findElement(element) {
            if (element && element.children && element.children.length > 0) {
                for (var i = 0; i < element.children.length; i++) {
                    ...
                    if (child.nodeName == "DIV" || child.nodeName == "IMG" && child.id || (child.nodeName == "SPAN" || child.nodeName == "DIV" || child.nodeName == "IMG" && child.className)) {
                        if (child.id || child.className) {
                            ar.push(child);
                        }
                    }
...
temple.banner = new temple.Banner();